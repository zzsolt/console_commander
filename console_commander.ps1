...
function Initialize-ConfigRuntime {
    ...
    $script:InputModePreference = [string]$script:Config.MouseMode
...
function Get-RenderLineKey {
    ...
    $segments = @($Line.Segments | Sort-Object -Property Left)
    foreach ($segment in $segments) {
        [void]$parts.Add(('{0}|{1}|{2}|{3}' -f $segment.Left, [int]$segment.Foreground, [int]$segment.Background, $segment.Text))
    }
...
function Get-BorderCharacters {
    if ([bool]$script:Config.UseAscii) {
...
function Add-MenuBarToLines {
    ...
    $line = $Lines[0]
    Add-RenderSegment -Line $line -Left 0 -Text '' -Width $Width -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
    $menus = @('Left', 'File', 'Command', 'Options', 'Right')
    $script:RenderState.TopMenuZones = @()
    $x = 1
    for ($i = 0; $i -lt $menus.Count; $i++) {
        $text = ' ' + $menus[$i] + ' '
        Add-RenderSegment -Line $line -Left $x -Text $text -Width $text.Length -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
        $script:RenderState.TopMenuZones += [pscustomobject]@{
            Name = $menus[$i]
            Index = $i
            Left = $x
            Right = $x + $text.Length - 1
            Top = 0
        }
        $x += $text.Length + 1
    }

    # English: Keep the right input badge away from menu hit zones on narrow consoles.
    # Magyar: Keskeny konzolon az input jelzo nem fedheti a menu kattintasi zonait.
    $availableRight = $Width - $x - 1
    $inputText = ' ' + (Get-InputStatusText) + ' '
    if ($inputText.Length -gt $availableRight) {
        $inputText = ' ' + (Get-ShortInputStatusText) + ' '
    }
    if ($inputText.Length -le $availableRight -and $availableRight -gt 4) {
        Add-RenderSegment -Line $line -Left ([Math]::Max($x, $Width - $inputText.Length)) -Text $inputText -Width $inputText.Length -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
    }
}
...
function Add-PanelToLines {
    ...
    $topBorder = $border.Corner + ($border.Horizontal * ($Width - 2)) + $border.Corner
    $bottomBorder = $topBorder
    Add-RenderSegment -Line $Lines[$Top] -Left $Left -Text $topBorder -Width $Width -Foreground $borderColor -Background ([ConsoleColor]::Black)
    Add-RenderSegment -Line $Lines[$Top + $Height - 1] -Left $Left -Text $bottomBorder -Width $Width -Foreground $borderColor -Background ([ConsoleColor]::Black)
    ...
    Add-RenderSegment -Line $Lines[$Top] -Left ($Left + 1) -Text $title -Width ($Width - 2) -Foreground $borderColor -Background ([ConsoleColor]::Black)

    for ($row = $Top + 1; $row -lt ($Top + $Height - 1); $row++) {
        Add-RenderSegment -Line $Lines[$row] -Left $Left -Text $border.Vertical -Width 1 -Foreground $borderColor -Background ([ConsoleColor]::Black)
        Add-RenderSegment -Line $Lines[$row] -Left ($Left + $Width - 1) -Text $border.Vertical -Width 1 -Foreground $borderColor -Background ([ConsoleColor]::Black)
    }
    ...
    $fixedWidth = 2 + 1 + 6 + 1 + 10 + 1 + 16 + 1 + 6
    $nameWidth = $innerWidth - $fixedWidth
    if ($nameWidth -lt 8) {
        $nameWidth = 8
    }

    $headerText = '{0} {1} {2,-6} {3,10} {4,-16} {5,-6}' -f 'SM', (Truncate-Text -Text 'Name' -Width $nameWidth), 'Type', 'Size', 'Modified', 'Attr'
    Add-RenderSegment -Line $Lines[$Top + 1] -Left $innerLeft -Text $headerText -Width $innerWidth -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
    ...
    for ($i = 0; $i -lt $visibleRows; $i++) {
        ...
        if ($itemIndex -lt $Panel.Items.Count) {
            ...
            $nameText = Truncate-Text -Text $item.Name -Width $nameWidth
            $lineText = '{0} {1} {2,-6} {3,10} {4,-16} {5,-6}' -f $markers, $nameText, $type, $size, $dateText, $attrs
        ...
        Add-RenderSegment -Line $Lines[$screenRow] -Left $innerLeft -Text $lineText -Width $innerWidth -Foreground $foreground -Background $background
    }

    $statusRow = $Top + $Height - 3
    $status = $Panel.Status
    ...
    Add-RenderSegment -Line $Lines[$statusRow] -Left $innerLeft -Text $status -Width $innerWidth -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)

    $scrollRow = $Top + $Height - 2
    $scroll = ('Selected {0}/{1}, Top {2}' -f ([Math]::Min($Panel.SelectedIndex + 1, $Panel.Items.Count)), $Panel.Items.Count, ($Panel.TopIndex + 1))
    Add-RenderSegment -Line $Lines[$scrollRow] -Left $innerLeft -Text $scroll -Width $innerWidth -Foreground ([ConsoleColor]::DarkGray) -Background ([ConsoleColor]::Black)
}
...
function Build-AppScreenLines {
    ...
    if ($Width -lt 60 -or $Height -lt 16) {
    ...
    Add-MenuBarToLines -Lines $lines -Width $Width

    $panelHeight = $Height - 3
    $leftWidth = [int][Math]::Floor($Width / 2)
    $rightWidth = $Width - $leftWidth

    $script:RenderState.LeftPanelZone = [pscustomobject]@{ Panel = 'Left'; Left = 0; Top = 1; Width = $leftWidth; Height = $panelHeight; RowTop = 3; VisibleRows = [Math]::Max(1, $panelHeight - 5) }
    $script:RenderState.RightPanelZone = [pscustomobject]@{ Panel = 'Right'; Left = $leftWidth; Top = 1; Width = $rightWidth; Height = $panelHeight; RowTop = 3; VisibleRows = [Math]::Max(1, $panelHeight - 5) }

    Add-PanelToLines -Lines $lines -Panel $script:State.LeftPanel -Left 0 -Top 1 -Width $leftWidth -Height $panelHeight -Active ($script:State.ActivePanel -eq 'Left')
    Add-PanelToLines -Lines $lines -Panel $script:State.RightPanel -Left $leftWidth -Top 1 -Width $rightWidth -Height $panelHeight -Active ($script:State.ActivePanel -eq 'Right')

    $commandPrompt = ('{0}> {1}' -f (Get-PathForDisplay -Path (Get-ActivePanel).Path), $script:State.CommandLine)
    Add-RenderSegment -Line $lines[$Height - 2] -Left 0 -Text $commandPrompt -Width $Width -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::Black)

    $keys = ' F1 Help  F2 User  F3 View  F4 Edit  F5 Copy  F6 Move  F7 Mkdir  F8 Delete  F9 PullDn  F10 Quit '
    Add-RenderSegment -Line $lines[$Height - 1] -Left 0 -Text $keys -Width $Width -Foreground ([ConsoleColor]::Black) -Background ([ConsoleColor]::Gray)
...
function Get-InputStatusText {
    ...
    if ($script:InputModePreference -eq 'KeyboardOnly') {
        return 'Input: Keyboard only'
    }
    if (-not [string]::IsNullOrWhiteSpace($script:MouseUnavailableReason)) {
        return 'Input: Keyboard only (mouse unavailable)'
    }
    return 'Input: Keyboard only'
}
...
function Show-InputModeStatus {
    $details = @(
        (Get-InputStatusText),
        '',
        'Mouse support is auto-enabled when the current console host supports Win32 console mouse input. Keyboard always remains available.'
    )
    if (-not [string]::IsNullOrWhiteSpace($script:MouseUnavailableReason)) {
...
function Initialize-ConsoleInputMode {
    try {
        $script:MouseInputAvailable = $false
        $script:MouseUnavailableReason = ''
        if ([string]$script:InputModePreference -eq 'KeyboardOnly') {
...
public class ConsoleCommanderNativeInput
{
    ...
    public static InputEvent ReadInputEvent(IntPtr handle)
    {
        ...
        while (true)
        {
            if (!ReadConsoleInput(handle, records, 1, out read) || read == 0)
            {
...
"@
        if ($null -eq ('ConsoleCommanderNativeInput' -as [type])) {
        ...
        if (-not [ConsoleCommanderNativeInput]::SetConsoleMode($handle, $newMode)) {
            throw 'SetConsoleMode failed'
        }

        $script:MouseInputAvailable = $true
        $script:MouseUnavailableReason = ''
        Write-AppLog -Level 'INFO' -Message 'Mouse input enabled through Win32 console input.'
    }
    catch {
        $script:MouseInputAvailable = $false
        $script:MouseUnavailableReason = $_.Exception.Message
        Write-AppLog -Level 'WARN' -Message ('Mouse input unavailable; keyboard-only mode active: {0}' -f $_.Exception.Message)
    }
    Request-FullRedraw
}
...
function Disable-MouseInputMode {
    ...
    $script:InputModePreference = 'KeyboardOnly'
    $script:Config.MouseMode = 'KeyboardOnly'
    $script:MouseUnavailableReason = ''
    Request-FullRedraw
    Show-Message -Message (Get-InputStatusText)
}
...
function Convert-NativeKeyEvent {
    ...
    $key = [ConsoleKey]$NativeEvent.VirtualKeyCode
    return New-Object ConsoleKeyInfo -ArgumentList $keyChar, $key, $shift, $alt, $control
}
...
function Read-InputEvent {
        ...
        while ($true) {
            try {
                ...
                if ($nativeEvent.Type -eq 'Resize') {
                    Request-FullRedraw
                    return [pscustomobject]@{ Type = 'Resize'; KeyInfo = $null; X = 0; Y = 0; ButtonState = 0; ButtonDown = $false; DoubleClick = $false; WheelDelta = 0 }
                }
                if ($nativeEvent.Type -eq 'Key') {
                    return [pscustomobject]@{ Type = 'Key'; KeyInfo = (Convert-NativeKeyEvent -NativeEvent $nativeEvent); X = 0; Y = 0; ButtonState = 0; ButtonDown = $false; DoubleClick = $false; WheelDelta = 0 }
                }
                if ($nativeEvent.Type -eq 'Mouse') {
                    $buttonDown = (($nativeEvent.ButtonState -band 0x00000001) -ne 0)
                    $doubleClick = ($nativeEvent.EventFlags -eq 0x0002)
                    $wheelDelta = 0
                    if ($nativeEvent.EventFlags -eq 0x0004) {
                    ...
                    return [pscustomobject]@{ Type = 'Mouse'; KeyInfo = $null; X = [int]$nativeEvent.X; Y = [int]$nativeEvent.Y; ButtonState = [uint32]$nativeEvent.ButtonState; ButtonDown = $buttonDown; DoubleClick = $doubleClick; WheelDelta = $wheelDelta; EventFlags = [uint32]$nativeEvent.EventFlags }
                }
            }
            catch {
                $script:MouseInputAvailable = $false
                $script:MouseUnavailableReason = $_.Exception.Message
                Write-AppLog -Level 'WARN' -Message ('Mouse input read failed; keyboard-only mode active: {0}' -f $_.Exception.Message)
                Request-FullRedraw
                break
            }
        }
    ...
    return [pscustomobject]@{ Type = 'Key'; KeyInfo = (Read-KeySafe); X = 0; Y = 0; ButtonState = 0; ButtonDown = $false; DoubleClick = $false; WheelDelta = 0 }
}
...
function Draw-DialogBox {
    ...
    $border = Get-BorderCharacters
    Write-At -Left $left -Top $top -Text ($border.Corner + ($border.Horizontal * ($boxWidth - 2)) + $border.Corner) -Width $boxWidth -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
    ...
    Write-At -Left $left -Top ($top + $boxHeight - 1) -Text ($border.Corner + ($border.Horizontal * ($boxWidth - 2)) + $border.Corner) -Width $boxWidth -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
...
function Show-ChoiceDialog {
    ...
    while ($true) {
        ...
        $border = Get-BorderCharacters
        Write-At -Left $left -Top $top -Text ($border.Corner + ($border.Horizontal * ($width - 2)) + $border.Corner) -Width $width -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
        ...
        Write-At -Left $left -Top ($top + $height - 1) -Text ($border.Corner + ($border.Horizontal * ($width - 2)) + $border.Corner) -Width $width -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
    ...
    try {
        ...
        if (-not $finished) {
            try {
                ...
                Write-AppLog -Level 'WARN' -Message ('Timed out command kill failed: {0}' -f $_.Exception.Message)
            }
            return @(
...
function Show-HelpViewer {
    $lines = @(
        ...
        '  Mouse support is auto-enabled when the current console host supports Win32 console mouse input. Keyboard always remains available.',
        '  Options can enable, disable, retry, or show the current input mode.',
        '  Input: KB+Mouse means combined input; Keyboard only means mouse disabled; Keyboard only (mouse unavailable) means fallback.',
        '',
        'Safety:',
...
function Get-TopMenuDefinitions {
    ...
    $optionItems = @(
        ...
        @{ Text = 'Enable mouse'; Action = 'EnableMouse' },
        @{ Text = 'Disable mouse'; Action = 'DisableMouse' },
        @{ Text = 'Retry mouse initialization'; Action = 'RetryMouse' },
        @{ Text = 'Show input mode status'; Action = 'InputStatus' },
...
function Draw-DropdownMenu {
    ...
    Write-At -Left $left -Top ($top + $visibleItems + 1) -Text ($border.Corner + ($border.Horizontal * ($width - 2)) + $border.Corner) -Width $width -Foreground ([ConsoleColor]::White) -Background ([ConsoleColor]::DarkBlue)
}
...
function Invoke-TopMenuAction {
    ...
    switch ($Item.Action) {
        ...
        'ToggleAscii' { $script:Config.UseAscii = -not [bool]$script:Config.UseAscii; Request-FullRedraw }
        ...
        'ReloadConfig' { $script:Config = Load-AppConfig; Refresh-Panel -Panel $script:State.LeftPanel; Refresh-Panel -Panel $script:State.RightPanel; Show-Message -Message 'Config reloaded.' }
        'ShowStubs' { Show-StubMenu }
        'About' { Show-Message -Message 'Mouse is auto-enabled when supported. Keyboard always remains available.' }
    }
...
function Show-StubMenu {
    ...
    $lines = @(
        ...
        'Mouse support: optional Win32 console input is implemented; keyboard-only fallback remains primary.',
...
function Handle-MouseEvent {
    ...
    if (-not $MouseEvent.ButtonDown -and -not $MouseEvent.DoubleClick) {
        return
    }

    if ($MouseEvent.ButtonDown -and -not $MouseEvent.DoubleClick) {
        $now = Get-Date
        if ($null -ne $script:LastMouseClick) {
            $elapsed = ($now - $script:LastMouseClick.Time).TotalMilliseconds
            if ($elapsed -lt 120 -and $script:LastMouseClick.X -eq $MouseEvent.X -and $script:LastMouseClick.Y -eq $MouseEvent.Y) {
                return
            }
        }
        $script:LastMouseClick = [pscustomobject]@{ X = $MouseEvent.X; Y = $MouseEvent.Y; Time = $now }
    }
...
function Run-SelfTest {
    ...
    try {
        ...
        $script:MouseInputAvailable = $false
        $script:MouseUnavailableReason = ''
        $keyboardStatusOk = ((Get-InputStatusText) -eq 'Input: Keyboard only')
        $script:InputModePreference = 'Auto'
        $script:MouseInputAvailable = $false
        $script:MouseUnavailableReason = 'test unavailable'
        $unavailableStatusOk = ((Get-InputStatusText) -eq 'Input: Keyboard only (mouse unavailable)')
        $script:MouseInputAvailable = $true
        $combinedStatusOk = ((Get-InputStatusText) -eq 'Input: KB+Mouse')
